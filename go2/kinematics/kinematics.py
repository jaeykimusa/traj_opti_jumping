...
def getDefaultStandStateFullOptimization(model, data, base_position=None, foot_target_positions=None):
    ...
    q_init = np.concatenate([base_position, base_rpy, joints])
...
# Alternative: If you want to optimize all values but ensure zero orientation
def getDefaultStandStateFullOptimizationFinal(model, data):
    
    base_position = np.array([0.0, 0.0, 0.3])
    foot_target_positions = {
        "LF_FOOT": np.array([2.1934, 0.142, 0]),
        "RF_FOOT": np.array([2.1934, -0.142, 0]),
        "LH_FOOT": np.array([-0.1934, 0.142, 0]),
        "RH_FOOT": np.array([-0.1934, -0.142, 0]),
    }
